-- ============================================================
-- Vehicle Rental Management System – MySQL Database
-- Server : XAMPP localhost (127.0.0.1:3306)
-- User   : root  |  Password: (empty by default)
-- Import : phpMyAdmin → Import → choose this file
--          OR: mysql -u root < VehicleRentalDB.sql
-- ============================================================
...
-- ── 2. Vehicles (Vehicle Management / Reports) ───────────────
CREATE TABLE IF NOT EXISTS vehicle (
    ID                  INT AUTO_INCREMENT PRIMARY KEY,
    Brand               VARCHAR(100) NOT NULL,
    Model               VARCHAR(100) NOT NULL,
    Type                VARCHAR(50)  NOT NULL,
    `Reg. No`           VARCHAR(20)  NOT NULL UNIQUE,
    `Daily Rate(LKR)`   DECIMAL(10,2) NOT NULL,
    `Manufacture Year`  INT          NOT NULL,
    Status              VARCHAR(50)  NOT NULL DEFAULT 'Available'
);

INSERT INTO vehicle (Brand, Model, Type, `Reg. No`, `Daily Rate(LKR)`, `Manufacture Year`, Status) VALUES
...
ON DUPLICATE KEY UPDATE `Reg. No` = `Reg. No`;

-- ── 3. Customers (CustomerForm / Payments) ───────────────────
...
INSERT INTO customers (FullName, NIC, Phone, LicenseNo, Address, DateRegistered, ActiveRentals) VALUES
...
-- ── 4. Rentals (Booking / Return / Payments / Reports) ───────
...
INSERT INTO rentals (CustomerID, VehicleID, StartDate, EndDate, TotalAmount, Status) VALUES
...
ON DUPLICATE KEY UPDATE RentalID = RentalID;

UPDATE customers SET ActiveRentals = (
    SELECT COUNT(*) FROM rentals r WHERE r.CustomerID = customers.CustomerID AND r.Status = 'Active'
);

-- ── 5. Payments (PaymentForm) ────────────────────────────────
...
INSERT INTO payments (RentalID, Customer, Vehicle, Period, BaseAmount, OverdueDays, PenaltyRate, LatePenalty, TotalDue, Discount, FinalAmount, Method, PayDate, Reference) VALUES
    (4, 'Nimali Perera',  'CBA-1010', '2025-01-01 → 2025-01-06', 20000, 0, 0, 0, 20000, 0, 20000, 'Cash', DATE_SUB(CURDATE(), INTERVAL 40 DAY), 'PAY-001'),
    (5, 'Kasun Silva',    'CBA-1009', '2025-02-01 → 2025-02-06', 24000, 0, 0, 0, 24000, 500, 23500, 'Card', DATE_SUB(CURDATE(), INTERVAL 25 DAY), 'PAY-002'),
    (6, 'Amila Fernando', 'CBA-3010', '2025-03-01 → 2025-03-06', 45000, 1, 500, 500, 45500, 0, 45500, 'Bank Transfer', DATE_SUB(CURDATE(), INTERVAL 15 DAY), 'PAY-003')
